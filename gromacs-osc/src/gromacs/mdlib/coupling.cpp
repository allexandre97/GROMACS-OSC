                      ...
                      double*           baros_integral)
{
    ...
    //MODIFICATION OF VANILLA BAROSTAT TO SIMULATE ULTRASOUNDS
 
    /* Calculate time from dt and step
     */
    real time;
    time = dt * step;

    /*Calculate pressure associated with sin wave
     */
    real  nu = 12E6; /*Frequency in Hz*/
    real  p = 35.0; /*Max Pressure in bar*/
    nu = nu/1E12;	/*Translate frequency to time units*/
    real Psin;

    Psin = p * sin(2 * M_PI * nu * time); /*Oscilating pressure calculation is done here*/
...
#define factor(d, m) (ir->compress[d][m] * dt / ir->tau_p)
    ...
    switch (ir->epct)
    {
        ...
        case epctSEMIISOTROPIC:
            ...
            mu[ZZ][ZZ] = 1.0 - factor(ZZ, ZZ) * (ir->ref_p[ZZ][ZZ] - pres[ZZ][ZZ] - Psin) / DIM;
            break;
        case epctANISOTROPIC:
            for (int d = 0; d < DIM; d++)
            {
